...
def clip_polygon(polygons, vertices_hole, junctions, meta, clip_holes=True):
    ...
    print(f"🔍 Starting clip_polygon with {len(polygons)} polygons and {len(vertices_hole)} hole vertices.")
    mesh_holes = []

    if len(polygons) == 1:
        print("🔹 Single polygon detected.")
        ...
        print(f"  - Triangulated wall vertices: {len(mesh_wall_vertices)}, faces: {len(mesh_wall_faces)}")
        return [mesh_wall_vertices, mesh_wall_faces], [mesh_wall_vertices, mesh_wall_faces], mesh_holes
    else:
        print("🔹 Multiple polygons detected.")
        wall = []
        holes = []
        print(f"  - Processing {len(polygons)} polygons.")
        for polygon in polygons:
            if np.any(np.intersect1d(polygon, vertices_hole)):
                print(f"  - Polygon {polygon} classified as a hole.")
                holes.append(polygon)
            else:
                print(f"  - Polygon {polygon} classified as a wall.")
                wall.append(polygon)

        # Extract junctions on this plane
        indices = []
        junctions_wall = []
        print(f"  - Processing wall with {len(wall)} polygons.")
        for plane in wall:
            for vertex in plane:
                indices.append(vertex)
                junctions_wall.append(junctions[vertex])
        print(f"  - Junctions for wall: {junctions_wall}")
        ...
        if len(mesh_wall_vertices) > 0:
            ...
            if clip_holes:
                ...
                for hole, hole_ids in zip(junctions_holes, hole_vertices):
                    print(f"  - Processing hole with {len(hole)} vertices.")
                    mesh_hole_vertices, mesh_hole_faces = triangulate(hole)
                    print(f"    - Triangulated hole vertices: {len(mesh_hole_vertices)}, faces: {len(mesh_hole_faces)}")
                    if len(mesh_hole_vertices) > 0:
                        try:
                            ...
                            print(f"    - Extruded wall and hole meshes created.")

                            # Apply transformations to the hole mesh
                            ...
                            print(f"    - Boolean difference completed.")

                            # Filter mesh
                            mask = np.argwhere(mesh_cut.vertices[:, 2] < 0.1)
                            ...
                            mesh_wall_faces_clipped = mesh_cut.faces
                            ...
                            print(f"❌ CalledProcessError: {err}")
            ...
            print(f"  - Returning clipped wall and holes.")
            return (
            ...
            print("❌ No wall vertices found. Returning empty results.")
            return (
...
def convert_lines_to_vertices(lines):
    ...
    polygons = []
    differing_elements = [line for line in lines if len(line) != 2]
    ...
    while len(lines) != 0:
        ...
        if lineID.size == 0 or juncID.size == 0:
            polygon = None
            continue
        #raise ValueError(f"BREAK")
