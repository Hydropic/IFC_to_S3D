...
def prepare_geometry_from_annos(annos, use_torch=True):
    ...
    for semantic in annos["semantics"]:
        ...
        if semantic["type"] in ["window", "door"]:
            ...
            print(f"################### planeID of {semantic['type']}: ",semantic["planeID"])
            for planeID in semantic["planeID"]:
                # with plane_id get coordinates of lines from planeLineMatrix and append to array
                #print("################### annos[planeLineMatrix][planeID]: ",annos["planeLineMatrix"][planeID])
                ...
                # print("################### 2",junction_pairs)
                #junction_pairs = [p for p in junction_pairs if len(p) == 2] # TEMPORARY FIX
                for p in junction_pairs:
                    if len(p) > 2:
                        print("################### >2",p)
                    elif len(p) < 2:
                ...
                print("################### 3",junction_pairs)
                #raise ValueError(f"BREAK")
                hole_polygons_labeled.append([convert_lines_to_vertices(junction_pairs), semantic["type"]])
    print("################### hole_polygons_labeled ",hole_polygons_labeled)
    hole_polygons_labeled = [(np.array(polygon[0]), label) for polygon, label in hole_polygons_labeled if polygon is not None and len(polygon)>0]
    print("################### hole_polygons_labeled",hole_polygons_labeled)
    print("################### hole_polygons_labeled2 ",*hole_polygons_labeled)
    ...
    for planeID in range(len(annos["planes"])):
        ...
        print("################### junction_pairs: ",junction_pairs)
        polygon = convert_lines_to_vertices(junction_pairs)
        print("################### polygon: ",polygon) # polygon enthält nur 2 Elemente und beide werden als "hole" in clip_polygon interpretiert statt "wall"
        print("################### junctions...........: ",junctions)
        print("################### vertices_holes: ",vertices_holes)
        ...
        for hole in holes:
            #print(hole)
            intersection_mask = np.array([
            np.any(np.isin(poly, hole[2])) for poly in hole_polygons])
    ...
    for semantic in annos["semantics"]:
        ...
        materials_clipped = []
        #print("polygons_semantic: ", polygons_semantic[4])
        # raise ValueError(f"BREAK")
    ...
    print("rooms", rooms)
    print("rooms_clipped", rooms_clipped)
    print("floors", floors)
    print("limits", limits)
    print("all_materials", all_materials)
    print("all_materials_clipped", all_materials_clipped)
    #raise ValueError(f"BREAK") # this is my way to break the code for debugging purposes
